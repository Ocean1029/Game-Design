...
/// <summary>
/// 互動提示按鍵效果組件 - 在物件上方顯示閃動的按鍵圖片
/// 當玩家靠近時自動顯示，離開時自動隱藏
/// 這是 InteractionPrompt 的複製版本，可以用於任何需要按鍵提示的物件
/// </summary>
[RequireComponent(typeof(Collider2D))]
public class InteractionPromptButton : MonoBehaviour
{
    [Header("按鍵圖片設定")]
    ...
    void Start()
    {
        ...
        {
            Debug.LogError($"InteractionPromptButton: {gameObject.name} 缺少 Collider2D 組件！");
            return;
        }
        ...
        if (showDebugInfo)
        {
            Debug.Log($"InteractionPromptButton: 初始化完成 - {gameObject.name}");
        }
    }
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        if (!useDistanceDetection && IsPlayer(collision))
        {
            ...
            if (showDebugInfo)
            {
                Debug.Log($"InteractionPromptButton: 玩家進入範圍 - {gameObject.name}");
            }
        }
    }
    ...
    void OnTriggerExit2D(Collider2D collision)
    {
        if (!useDistanceDetection && IsPlayer(collision))
        {
            ...
            if (showDebugInfo)
            {
                Debug.Log($"InteractionPromptButton: 玩家離開範圍 - {gameObject.name}");
            }
        }
    }
    ...
    private void LoadButtonSprite()
    {
        ...
        if (buttonSprite == null)
        {
            if (showDebugInfo)
            {
                Debug.LogWarning($"InteractionPromptButton: 無法載入按鍵圖片，請在 Inspector 中手動設置！路徑：Assets/Image/UI/Z_but.png");
            }
        }
    }
    ...
    private void CreatePromptObject()
    {
        ...
        promptObject = new GameObject("InteractionPromptButton_Button");
